...
AddEventHandler("bankrobberies:eventHandler", function(event, eventData)
	...
	elseif event == "alarm_police" then
		if ESX.PlayerData["job"] and ESX.PlayerData["job"]["name"] == "police" then
			...
			ESX.ShowNotification("Quelqu'un est en train de Braquer la - ~g~ " .. eventData .. " ~s~Depechez Vous")
		...
		for bank, values in pairs(Config.Bank) do
			...
			if dstCheck <= 5.0 then
				...
				if dstCheck <= 1.0 then
					local usable, displayText = not cachedData["banks"][bank], cachedData["hacking"] and "~r~Hack..." or cachedData["banks"][bank] and "~r~Braquage~s~ en cours . . ." or "~INPUT_CONTEXT~ Démarrer ~r~braquage~s~ avec le PC."
